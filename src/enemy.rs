...
impl Plugin for EnemyPlugin {
	fn build(&self, app: &mut App) {
		app.add_startup_system(enemy_spawn_system).add_system(enemy_movement_system);
	}
}

fn enemy_spawn_system(
	mut commands: Commands, texture: Res<CharacterTextures>
) {
    let enemies = vec!(Vec3::new(1., 1., 0.), Vec3::new(7., 1., 0.), Vec3::new(28., 11., 1.));
...
fn enemy_movement_system(mut enemy_query: Query<(&mut Enemy, &mut Transform)>, time: Res<Time>,
wall_query: Query<&Transform, (With<WallColider>, Without<Enemy>)>,) {
	for (mut enemy, mut transform) in enemy_query.iter_mut() {
		if enemy.last_movement + MINIMUM_ENEMY_MOVEMENT_BREAK <= time.seconds_since_startup() as f32 {
            ...
            let new_exact_position = round_position(transform.translation.clone() + Vec3::new(x_delta, y_delta, 0.0));
